...
Sub FetchSalesData()
    ...
    If InStr(1, basePath, "https", vbTextCompare) > 0 Then
        ...
        If Dir(basePath & sep & "OneDrive", vbDirectory) <> "" Then
            basePath = basePath & sep & "OneDrive"
        End If
        If Dir(basePath & sep & "Desktop", vbDirectory) <> "" Then
            basePath = basePath & sep & "Desktop"
        End If
        If Dir(basePath & sep & "デスクトップ", vbDirectory) <> "" Then
            basePath = basePath & sep & "デスクトップ"
        End If
        If Dir(basePath & sep & "売上メール", vbDirectory) <> "" Then
    ...
    dp = ""
    Dim subF As String
    subF = Dir(basePath & sep & "*", vbDirectory)
    Do While subF <> ""
        If subF <> "." And subF <> ".." Then
            On Error Resume Next
            Dim attr As Long
            attr = GetAttr(basePath & sep & subF)
            On Error GoTo 0
            If (attr And vbDirectory) <> 0 Then
                Dim testFile As String
                testFile = basePath & sep & subF & sep & "売上管理表.xlsx"
                If Dir(testFile) <> "" Then dp = testFile: Exit Do
            End If
        End If
        subF = Dir()
    Loop
    If dp = "" Then dp = basePath & sep & "データベース" & sep & "売上管理表.xlsx"
    ...
    For r = fc + 1 To 3 Step -1
        If wsDay.Cells(r, 1).Value <> wsDay.Cells(r - 1, 1).Value And wsDay.Cells(r - 1, 1).Value <> "" Then
            ...
            With wsDay.Range(wsDay.Cells(r, 1), wsDay.Cells(r, 7))
                .Interior.Color = RGB(218, 230, 242)
                .HorizontalAlignment = xlCenter: .VerticalAlignment = xlCenter
                .Borders.LineStyle = xlContinuous
                .Borders.Weight = xlThin
                .Borders.Color = RGB(180, 198, 220)
            End With
            wsDay.Rows(r + 1).Insert Shift:=xlDown
            wsDay.Rows(r + 1).Interior.ColorIndex = xlNone
            wsDay.Rows(r + 1).Borders.LineStyle = xlNone
        End If
    Next r
    ...
    If dataLast >= 2 Then
        ...
        wsDay.Cells(dataLast + 1, 3).Value = "日合計"
        wsDay.Cells(dataLast + 1, 3).Font.Bold = True
        dayKey = ""
        Dim sr As Long
        ...
        Next sr
        If dicDayAll.Exists(dayKey) Then
            wsDay.Cells(dataLast + 1, 6).Value = dicDayAll(dayKey)
            wsDay.Cells(dataLast + 1, 6).Font.Bold = True
        End If
    ...
    With wsTot.Range("A1")
        ...
        .HorizontalAlignment = xlCenter: .RowHeight = 30
        .Interior.Color = RGB(240, 245, 252)
    End With
    ...
    For r = startRow + 1 To lastRow + 1
        ...
        If ws.Cells(r, col).Value <> ws.Cells(r - 1, col).Value Or r > lastRow Or isSep Then
            If r - 1 > sm And ws.Cells(sm, col).Value <> "" Then
                ...
                ws.Cells(sm, col).HorizontalAlignment = xlCenter
                ws.Cells(sm, col).VerticalAlignment = xlCenter
            End If
            sm = r
        ...
        If storeIdx Mod 2 = 1 Then
            For c = 1 To 7
                If ws.Cells(r, c).MergeCells Then
                    ws.Cells(r, c).MergeArea.Interior.Color = fillClr
                Else
...
Sub MakeChart(fd As Variant, fc As Long, sd As Date, ed As Date, siteFilter As String)
    ...
    For i = 1 To fc
        ...
        If dicDate.Exists(dKey) Then
            dicDate(dKey) = dicDate(dKey) + CDbl(fd(i, 6))
        Else
            dicDate.Add dKey, CDbl(fd(i, 6))
            dicTemp.Add dKey, 0: dicTempCnt.Add dKey, 0
        End If
        If fd(i, 7) <> "" And IsNumeric(fd(i, 7)) Then
            dicTemp(dKey) = dicTemp(dKey) + CDbl(fd(i, 7))
            dicTempCnt(dKey) = dicTempCnt(dKey) + 1
        End If
    Next i
    ...
    With cht.SeriesCollection(1)
        .Name = "売上金額": .Format.Fill.ForeColor.RGB = RGB(31, 78, 120)
    End With
    If cht.SeriesCollection.Count >= 2 Then
        With cht.SeriesCollection(2)
            .Name = "最高気温": .ChartType = xlLine: .AxisGroup = xlSecondary
            .Format.Line.ForeColor.RGB = RGB(255, 102, 0): .Format.Line.Weight = 2.5
        End With
    End If
...
Sub UpdateChart()
    ...
    Dim curDate As String, curStore As String, cellSub As Double
    curDate = "": curStore = ""
    Dim lastRD As Long: lastRD = lastR
    For i = 2 To lastRD
        ...
        ElseIf StrComp(axis, curStore, vbTextCompare) = 0 Then
            grpKey = curDate
        ElseIf StrComp(axis, CStr(wsDay.Cells(i, 3).Value), vbTextCompare) = 0 Then
            grpKey = curDate
        End If
